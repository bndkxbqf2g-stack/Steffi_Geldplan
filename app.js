...
function holidays(y){var e=easter(y),a=[dateKey(new Date(y,0,1)),dateKey(new Date(y,0,6)),dateKey(new Date(y,4,1)),dateKey(new Date(y,7,15)),dateKey(new Date(y,9,3)),dateKey(new Date(y,10,1)),dateKey(new Date(y,11,25)),dateKey(new Date(y,11,26))];[-2,1,39,50,60].forEach(function(n){a.push(dateKey(addDays(e,n)));});return a;}
...
var DEFAULT_FIX=[
 ...
 {id:"d-ticket",name:"Deutschlandticket",amount:63},
...
function sortFixItems(a){return a.slice().sort(function(left,right){return (Number(right.amount)||0)-(Number(left.amount)||0);});}
function fixItems(){try{var raw=localStorage.getItem("meinGeldplanFixItems");var a=raw?JSON.parse(raw):null;if(sameOldDefault(a)){var migrated=sortFixItems(DEFAULT_FIX.map(function(x){return {id:x.id,name:x.name,amount:x.amount};}));saveFixItems(migrated);return migrated;}if(Array.isArray(a)&&a.length)return sortFixItems(a);}catch(e){}return sortFixItems(DEFAULT_FIX.map(function(x){return {id:x.id,name:x.name,amount:x.amount};}));}
